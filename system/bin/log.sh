...
do
    ...
    #soc监控
    soc_max_freq_Current=/sdcard/TC/Result/PTC/soc_max_freq_Current
    #当前电流
    ...
    #日志合集
    #未除温控
    if [ $thermal_engine_zf -gt 0 ]; then
        if [ $Charge -ge 0 ]; then
	        echo "没充电"
...
当前电池电流=`expr $Charge / 1000`毫安" > /sdcard/TC/日志.log
            echo "执行完毕"
...
当前最大充电电流=`expr $recharging_current_freq_file / 1000`毫安" > /sdcard/TC/日志.log
            echo "执行完毕"
        fi
    #除去温控
        ...
        #gpu检测间隔
        gpu_time=/sdcard/TC/parameter/PTC/gpu_time
        #处理器降频幅度
        ...
        #充电减小满血温度阈值
        Increase_current_threshold=`cat /sdcard/TC/parameter/CTC/Increase_current_threshold`
        Lower_current_threshold=`cat /sdcard/TC/parameter/CTC/Lower_current_threshold`
        #每次减少电流阈值
        Reduce_current_size=/sdcard/TC/parameter/CTC/Reduce_current_size
        #检测电池温度间隔
        charge_time=/sdcard/TC/parameter/CTC/charge_time
        #充电最大最小电流
        Minimum_current=/sdcard/TC/parameter/CTC/Minimum_current
        Maximum_current=/sdcard/TC/parameter/CTC/Maximum_current
            if [ $Charge -ge 0 ]; then
            echo "当前时间：[ `date +"%Y年%m月%d日 %H点%M分%S秒 $Z"` ]
当前处理器状态
当前`cat /sdcard/TC/Result/PTC/soc_present.log`
`cat $soc_max_freq_Current`
...
阶梯减少充电阈值=`cat $Reduce_current_size`毫安" > /sdcard/TC/日志.log
            echo "执行完毕"
        elif [ $Charge -lt 0 ]; then
            echo "当前时间：[ `date +"%Y年%m月%d日 %H点%M分%S秒 $Z"` ]
当前处理器状态
...
当前正在充电
当前电池温度=`echo "$battery_temp 10"|awk '{print ($1/$2)}'`℃
当前`cat /sdcard/TC/Result/CTC/current_present.log`
当前`cat /sdcard/TC/Result/CTC/current_present_mode.log`
当前充电电流=`expr $Charge / -1000`毫安
