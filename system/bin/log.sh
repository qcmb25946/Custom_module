...
当前处理器状态
当前`cat /storage/emulated/TC/Result/PTC/soc_present.log`
...
阶梯减少充电阈值=`cat $Reduce_current_size`毫安" > /storage/emulated/TC/日志.log
            echo "执行完毕"
...
当前正在充电
当前电池温度=`echo "$battery_temp 10"|awk '{print ($1/$2)}'`℃
当前`cat /storage/emulated/TC/Result/CTC/current_present.log`
当前`cat /storage/emulated/TC/Result/CTC/current_present_mode.log`
...
当前未充电
当前电池温度=`echo "$battery_temp 10"|awk '{print ($1/$2)}'`℃
当前电池电流=`expr $Charge / 1000`毫安" > /storage/emulated/TC/日志.log
                echo "执行完毕"
            elif [ $Charge -lt 0 ]; then
	            echo "正在充电"
                echo "当前时间：[ `date +"%Y年%m月%d日 %H点%M分%S秒 $Z"` ]
